...
def go(config: DictConfig):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if "basic_cleaning" in active_steps:
            # Download file, perform cleaning, upload cleaned data
            _ = mlflow.run(
        ...
        if "data_check" in active_steps:
            # do some data checks using pytest
            _ = mlflow.run(
        ...
        if "data_split" in active_steps:
            # reuse an existing component to split the given dataset
            _ = mlflow.run(
        ...
        if "test_regression_model" in active_steps:
            # use of the adapted local component in order to avoid version conflicts
            _ = mlflow.run(
